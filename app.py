import os, sqlite3, uuid, time, hashlib, hmac, secrets
...
def crear_pedido():
    ...
    if c.postgres:
        cur=c.execute(
            "INSERT INTO orders(business_id,user_id,customer_name,customer_phone,customer_address,customer_email,notes,payment_method,payment_status,total,subtotal,delivery_fee,commission_rate,commission_amount,status,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) RETURNING id",
            (bid,user_id,name,phone,address,email,notes,payment,"pending" if payment in ("Tarjeta (PayU)","Tarjeta (Conekta)") else "not_required",grand_total,total,delivery_fee,commission_rate,commission_amount,"nuevo",now)
        )
        oid=cur.fetchone()["id"]
    else:
        cur=c.execute(
            "INSERT INTO orders(business_id,user_id,customer_name,customer_phone,customer_address,customer_email,notes,payment_method,payment_status,total,subtotal,delivery_fee,commission_rate,commission_amount,status,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (bid,user_id,name,phone,address,email,notes,payment,"pending" if payment in ("Tarjeta (PayU)","Tarjeta (Conekta)") else "not_required",grand_total,total,delivery_fee,commission_rate,commission_amount,"nuevo",now)
        )
        oid=cur.lastrowid
