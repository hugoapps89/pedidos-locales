...
def favoritos():
    if not session.get("user_logged"):
        return redirect(
            url_for(
                "google_login",
                next=url_for("favoritos")
            )
        )

    c=db()
    ensure_user_schema(c)
    c.close()
    ...
    (business_id,)
        ...
        if (
            ...
            return jsonify(
                ok=False,
                error=(
                    "Este negocio aún no tiene configurada "
                    "su ubicación para calcular el envío."
                )
            ), 409
...
def crear_pedido():
    ...
    if requested_coupon:
        ...
        # Verificar que aplique al negocio
        coupon_business_id = coupon["business_id"]

        if (
            coupon_business_id is not None
            and int(coupon_business_id) != bid
        ):
            c.close()
            return jsonify(
                ok=False,
                error="Este cupón no aplica para este negocio."
            ), 400

        # Verificar vigencia
        today = date.today().isoformat()

        if today < str(coupon["start_date"]):
            c.close()
            return jsonify(
                ok=False,
        ...
        if total < minimum_purchase:
            ...
            return jsonify(
                ok=False,
                error=(
                    f"La compra mínima para este cupón "
                    f"es de ${minimum_purchase:.2f}."
                )
            ), 400
        ...
        if int(coupon["one_per_customer"] or 0):
    ...
    now = datetime.now(LOCAL_TZ).replace(
        tzinfo=None
    ).isoformat(timespec="seconds")

    payment_status = (
        "pending"
        if payment in ("Tarjeta (PayU)", "Tarjeta (Conekta)")
        else "not_required"
    )
...
def admin_coupon_delete(coupon_id):
    ...
    one_per_customer = (
        1 if request.form.get("one_per_customer") else 0
    )

    if not code:
        flash("Escribe un código para el cupón.", "error")
        return redirect(url_for("admin_coupons"))

    if discount_type not in {"percent", "fixed"}:
        flash("Tipo de descuento inválido.", "error")
        return redirect(url_for("admin_coupons"))

    if discount_value <= 0:
        flash("El valor del descuento debe ser mayor que cero.", "error")
        return redirect(url_for("admin_coupons"))

    if discount_type == "percent" and discount_value > 100:
        flash("El porcentaje no puede ser mayor a 100%.", "error")
        return redirect(url_for("admin_coupons"))

    if minimum_purchase < 0:
        flash("La compra mínima no puede ser negativa.", "error")
        return redirect(url_for("admin_coupons"))

    if max_uses is not None and max_uses < 1:
        flash("El máximo de usos debe ser mayor que cero.", "error")
        return redirect(url_for("admin_coupons"))

    if not start_date or not end_date:
        flash("Debes indicar las fechas del cupón.", "error")
        return redirect(url_for("admin_coupons"))
    ...
    flash(
        f"Cupón {code} creado correctamente.",
        "success"
    )
...
def validar_cupon():
    ...
    try:
        ...
        return jsonify(
            ok=False,
            error="Datos del cupón inválidos."
        ), 400
    ...
    # Verificar que el cupón corresponda al negocio
    coupon_business_id = coupon["business_id"]

    if (
        coupon_business_id is not None
        and int(coupon_business_id) != business_id
    ):
        c.close()
        return jsonify(
            ok=False,
